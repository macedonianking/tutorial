...
void main_options_call_help() {
	...
	fprintf(stdout,
			"    --non-constant-id: generate non constant R.java file\n");
}

int main_options_initialize_from_arguments(struct main_options *options,
		int argc, char **argv) {
	int i;
	int r;
...
int main_options_check_statements(struct main_options *options) {
	...
	if (options->v_packages.n <= 0
			|| options->v_packages.n != options->v_resource_r_files.n) {
		return -1;
	}
